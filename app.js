...
function renderBackground() {
    ...
    petals.forEach(p => {
        p.y += p.speedY;
        p.angleOsc += p.oscillationSpeed;
        p.x += p.speedX + Math.sin(p.angleOsc) * 0.25;
...
function initIntroBalloons() {
    ...
    const headerBottom = 240; // bottom of the fixed header (pink dashed line)
    const centerX = w / 2;
    const cardHalfWidth = 170;
    const cardTop = 390;

    // Generate 30 balloons programmatically scattered across the full screen width
    ...
    for (let i = 0; i < 30; i++) {
        ...
        // Double in size (100px to 136px)
        const size = 100 + Math.floor(Math.random() * 36);
...
function updateIntroBalloons() {
    ...
    const centerX = w / 2;
    const margin = 10;
    const headerBottom = 240; // Height of the fixed header line
    const cardHalfWidth = 170; // Half width of the gift card avoidance box
    const cardTop = 390;      // Top Y edge of the gift card avoidance box
    ...
    for (let i = 0; i < introBalloons.length; i++) {
        ...
        for (let j = i + 1; j < introBalloons.length; j++) {
            ...
            if (dist < minDist && dist > 0) {
                ...
                const pushX = Math.cos(angle) * overlap * 0.05;
                const pushY = Math.sin(angle) * overlap * 0.05;
                b1.vx -= pushX;
                b1.vy -= pushY;
                b2.vx += pushX;
                b2.vy += pushY;
    ...
    introBalloons.forEach(b => {
        ...
        b.swing += (b.targetSwing - b.swing) * 0.08;

        // Animate wavy string based on velocity and float phase
        b.wavePhase += 0.08 + speed * 0.1;
...
window.addEventListener('resize', () => {
    // re-scatter balloons nicely inside the margins on window resize
    const w = window.innerWidth;
    const h = window.innerHeight;
